...
const InvoiceData = () => {
    ...
    const { name, supplier } = router.query;
    ...
    const handleAddCustomer = async (e) => {
        ...
        const requestData = {
            data: {
                ...
                "is_return": 1,
                "supplier": customerData.return_invoice,
                "update_stock": "1",
                "docstatus": "1",
    ...
    async function fetchCustomerList(name, supplier) {

        const authHeader = getAuthHeader()
        try {
            const res = await axios.get('https://tgc67.online/api/resource/Purchase%20Invoice/' + name, authHeader)
    ...
    useEffect(() => {
        ...
        if (name && supplier) {
            fetchCustomerList(name, supplier)
        }
    }, [name, supplier])
                            ...
                            <div className="card mb-3" style={{ position: 'relative' }}>
                                <div className="card-body">
                                    ...
                                    <form onSubmit={handleAddCustomer} method="post" className="row g-3 needs-validation">
                                        ...
                                        <div className="w-100">
                                            <button className="btn btn-primary login-btn" type="submit">Return Purchase</button>
                                        </div>
                                    </form>
                                </div>
                            </div>
